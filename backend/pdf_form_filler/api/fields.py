...
from pdf_form_filler.api.schemas import (
    FieldValueUpdate,
    ImageFieldCreate,
    ImageFieldUpdate,
)
...
def list_fields(document_id):
    try:
        return {"fields": service.list_fields(document_id)}
...
def add_field(document_id, payload: ImageFieldCreate):
    try:
        ...
        # API uses "id"; service uses "field_id".
        data["field_id"] = data.pop("id")

        return service.add_image_field(
            document_id,
            **data,
        )
...
def update_field(
    document_id,
    field_id,
    payload: ImageFieldUpdate,
):
    try:
        return service.update_image_field(
            document_id,
            field_id,
...
def delete_field(document_id, field_id):
    try:
        return service.delete_image_field(
            document_id,
            field_id,
        )
    except Exception as error:
...
def set_value(
    ...
    try:
        return service.set_field_value(
            document_id,
            field_id,
            payload.value,
        )
